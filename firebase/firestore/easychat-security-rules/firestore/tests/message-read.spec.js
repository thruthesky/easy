...
const { db, a, b, c, tempChatRoomData, createChatRoom, block, unblock, invite } = require("./setup");
...
describe("Message read test", () => {
    ...
    it("Read Group chat if the user is unblocked - success", async () => {
        ...
        // user A unblocked B
        await unblock(a, b, roomRef.id);

        // user B should see the messages
        await firebase.assertSucceeds(
            db(b)
                .collection("easychat")
                .doc(roomRef.id)
    ...
    it("Read Group chat if another user is blocked - success", async () => {
        ...
        // user A invited user C
        await invite(a, c, roomRef.id);

        // user A blocked B
        await block(a, b, roomRef.id);

        // user C should see the messages
        await firebase.assertSucceeds(
            db(c)
                .collection("easychat")
                .doc(roomRef.id)
